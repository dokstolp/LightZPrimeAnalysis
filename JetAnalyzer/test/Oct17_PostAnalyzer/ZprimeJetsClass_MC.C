		    ...
		    std::cout<<"Before electron veto:jetveto.size(): "<<jetveto.size()<<std::endl;
		        ...
		        for(int j = 0; j < jetveto.size(); j++)
			   {
				...
				std::cout<<run<<":"<<lumis<<":"<<event<<std::endl;
                                if (j1etaWidth<0.04)
                                  {
  				    std::cout<<"events after j1etaWidth cut"<<std::endl;
				    std::cout<<run<<":"<<lumis<<":"<<event<<std::endl;
                                    h_deltar_cut->Fill(deltar);
				    pfMET_prof_after->Fill(deltar,pfMET,1);
				    h_trk12ptratio_afterdR->Fill(j1trk12PT/j1PT);
                                    fillHistos(8); 
...
void ZprimeJetsClass_MC::BookHistos(const char* file2)
{
  ...
  for(int i=0; i<11; i++){
     ...
     h_j1ntracks[i] = new TH1F(("j1nTracks"+histname).c_str(),"j1nTracks: Number of tracks associated with Leading Jet",20,0,20);h_j1ntracks[i]->Sumw2();
     h_j2ntracks[i] = new TH1F(("j2nTracks"+histname).c_str(),"j2nTracks: Number of tracks associated with Second Leading Jet",20,0,20);h_j2ntracks[i]->Sumw2();
     h_j1trk12pt[i] = new TH1F(("j1trk12pt"+histname).c_str(),"j1trk12pt: Sum of P_{T} of two leading tracks associated with Leading Jet",20,0,200);h_j1trk12pt[i]->Sumw2();
...
void ZprimeJetsClass_MC::fillHistos(int histoNumber)
{
  ...
  h_j1ntracks[histoNumber]->Fill(j1nTracks);
  h_j2ntracks[histoNumber]->Fill(j2nTracks);
  h_j1trk12pt[histoNumber]->Fill(j1trk12PT);
...
std::vector<int> ZprimeJetsClass_MC::JetVetoDecision(int jet_index) {
